...
C:\lesovod\
  ...
  app-release\      ← version.json + .apk мобильного приложения (см. п.8)
  caddy\caddy.exe
  nssm\nssm.exe
...
## 8. Обновления мобильного приложения «Лесовод Мобайл» (сайдлоуд APK)

Приложение ставится сайдлоудом (не через Google Play), поэтому
проверку обновлений делает само приложение: при запуске незаметно
запрашивает `version.json`, сравнивает `version_code` со своей сборкой
и, если на сервере новее, показывает баннер «Доступна новая версия» со
ссылкой на `apk_url`. Всё это — чисто статика, Caddy отдаёт файлы как
есть (см. блок `handle_path /app/*` в `Caddyfile`), backend в этом не
участвует вообще, обновлять файлы — только руками, при каждом релизе.

**Разовая настройка (один раз на сервере):**

1. Создайте папку `C:\lesovod\app-release\` (см. разметку в п.0).
2. Скопируйте `deploy\version.json.example` → `C:\lesovod\app-release\version.json`.
3. Положите собранный APK туда же под именем `lesovod-latest.apk` —
   именно это имя зашито в `apk_url` из примера; если назовёте файл
   иначе, поправьте `apk_url` в `version.json` соответственно.
4. Перезапуск Caddy/backend не нужен — `file_server` читает файлы с
   диска на каждый запрос.

**При каждом новом релизе приложения:**

1. Соберите новый APK, замените им `C:\lesovod\app-release\lesovod-latest.apk`
   (то же имя файла — `apk_url` в `version.json` менять не придётся).
2. Откройте `C:\lesovod\app-release\version.json`, увеличьте
   `version_code` на число БОЛЬШЕЕ, чем `BuildConfig.VERSION_CODE`
   только что собранного APK (обычно — то же самое значение) — именно
   по этому числу приложения у пользователей поймут, что вышла новая
   версия.
3. Проверьте вручную: `curl https://<адрес-сервера>/app/version.json`
   должен отдать новый `version_code`, `curl -I .../app/lesovod-latest.apk`
   — 200 и актуальный размер файла.

Ничего сложнее не нужно — ни отдельного сервиса, ни бэкенд-эндпоинта,
ни базы данных под версии: один статический JSON и один файл,
перезаписываемые руками.
